...
describe('POST /api/products/:id/sold', () => {
  it('should create a product, get a reservation token, reserve a product, sell a product, check valid amounts for instock, reserved, sold', async () => {
    ...
    const res5thCall = await request(app).post('/api/products/' + productId +'/sold').send(rtoken);
    ...
    expect(res6thCall.body.IN_STOCK).to.equal(122);
    expect(res6thCall.body.RESERVED).to.equal(0);
    expect(res6thCall.body.SOLD).to.equal(1);
  });
});

describe('POST /api/products/:id/sold', () => {
  it('should create a product, get a reservation token, sell twice the same product, check valid amounts for instock, reserved, sold', async () => {
    let productId: string = makeFakeProductId();
    let stock = {
      stock: 123,
    }
    ...
    const res7thCall = await request(app).post('/api/products/' + productId +'/sold').send(rtoken);
    expect(res7thCall.status).to.equal(400);
    expect(res7thCall.body.message).to.be.equal('product is already sold');
  });
});
